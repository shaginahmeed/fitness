...
const WorkoutHistory = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_BACKEND_URL}/api/workoutlog/${userName}`
        );
  ...
  const processData = (workoutData) => {
    ...
    const pastSevenDaysData = workoutData.filter(
      (data) =>
        new Date(data.workoutDate) >
        new Date(today.getTime() - 7 * 24 * 60 * 60 * 1000)
    );
    const pastSixMonthsData = workoutData.filter(
      (data) =>
        new Date(data.workoutDate) >
        new Date(today.getFullYear(), today.getMonth() - 6, today.getDate())
    );

    const workoutMinutesByDay = aggregateData(
      pastSevenDaysData,
      "workoutDate",
      "workoutMinutes"
    );
    const workoutMinutesByMonth = aggregateData(
      pastSixMonthsData,
      (data) =>
    ...
    setWorkoutDayData(
      generateChartData(workoutMinutesByDay, "Workout Minutes")
    );
    setWorkoutMonthData(
      generateChartData(workoutMinutesByMonth, "Workout Minutes")
    );

    const todayWorkouts = workoutData.filter(
      (data) => data.workoutDate === today.toISOString().slice(0, 10)
    );
    const pastMonthData = workoutData.filter(
      (data) =>
        new Date(data.workoutDate) >
        new Date(today.getFullYear(), today.getMonth() - 1, today.getDate())
    );

    const todayWorkoutMinutesByType = aggregateData(
      todayWorkouts,
      "workoutType",
      "workoutMinutes"
    );
    const pastMonthWorkoutMinutesByType = aggregateData(
      pastMonthData,
      "workoutType",
      "workoutMinutes"
    );

    setWorkoutTypeDayData(
      generateChartData(
        todayWorkoutMinutesByType,
        "Workout Minutes",
        "doughnut"
      )
    );
    setWorkoutTypeMonthData(
      generateChartData(
        pastMonthWorkoutMinutesByType,
        "Workout Minutes",
        "doughnut"
      )
    );
  ...
  useEffect(() => {
    setDataForWorkoutTotal(
      displayMode === "Days" ? workoutDayData : workoutMonthData
    );
    setDataForWorkoutType(
      displayModeType === "Days" ? workoutTypeDayData : workoutTypeMonthData
    );
  }, [
    displayMode,
    displayModeType,
      ...
      {showNotification ? (
        ...
        <div className="workout-container-1 workout-container-history">
          <div className="workout-history-container">
            ...
            {dataForWorkoutTotal &&
              Object.keys(dataForWorkoutTotal).length > 0 && (
          ...
          <div className="workout-history-container">
            <div className="button-title-container">
              <button
                className="toggle-button-water"
                onClick={toggleDaysMonthsType}
              >
                {displayModeType}
              </button>
              <h2 className="water-history-title">
                {displayModeType === "Days"
                  ? "Workout History by Days"
                  : "Workout History by Months"}
              </h2>
            </div>
            {dataForWorkoutType &&
              Object.keys(dataForWorkoutType).length > 0 && (
                <div className="chart-container">
